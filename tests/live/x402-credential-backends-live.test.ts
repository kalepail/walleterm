...
const SSH_AGENT_SYSTEM_ENABLED =
  SSH_AGENT_ENABLED && process.env.WALLETERM_LIVE_SSH_AGENT_SYSTEM === "1";
...
`,
    ...
    console.log(`[live.x402-op] payer=${payer.publicKey()} usdc_balance=${usdcBalance}`);
  }, 180_000);
  ...
  it("pays exact (direct) route with op:// raw secret", { timeout: 240_000 }, async () => {
...
describe.skipIf(!KEYCHAIN_ENABLED)(
  "walleterm live x402 — macOS Keychain raw secret (keychain://)",
  () => {
    let configPath: string;
    let secretRef: string;
    let payer: Keypair;
    ...
    beforeAll(async () => {
      ...
      const usdcBalance = await ensureTestnetUsdcBalance(payer);
      console.log(`[live.x402-keychain] payer=${payer.publicKey()} usdc_balance=${usdcBalance}`);
    }, 180_000);
    ...
    it("pays exact (direct) route with keychain:// raw secret", { timeout: 240_000 }, async () => {
      const result = await runPayJson(
        configPath,
        secretRef,
        `${X402_NFT_BASE_URL}/mint/attractor?size=64&seed=${uniqueSeed()}`,
...
describe.skipIf(!SSH_AGENT_SYSTEM_ENABLED)("walleterm live x402 — system SSH agent", () => {
  ...
  beforeAll(async () => {
    ...
    stellarAddress =
      identity.comment.startsWith("G") && identity.comment.length === 56
        ? identity.comment
        : await (async () => {
    ...
    console.log(`[live.x402-ssh-system] payer=${stellarAddress} usdc_balance=${usdcBalance}`);
  }, 300_000);
  ...
  it("pays exact (direct) route with system SSH agent", { timeout: 240_000 }, async () => {
    const result = await runPayJson(
      configPath,
      secretRef,
      `${X402_NFT_BASE_URL}/mint/attractor?size=64&seed=${uniqueSeed()}`,
...
describe.skipIf(!SSH_AGENT_1P_ENABLED)("walleterm live x402 — 1Password SSH agent", () => {
  ...
  beforeAll(async () => {
    ...
    const usdcBalance = await ensureUsdcForSshAgentKey(stellarAddress, secretRef);
    console.log(`[live.x402-ssh-1p] payer=${stellarAddress} usdc_balance=${usdcBalance}`);
  }, 300_000);
  ...
  it("pays exact (direct) route with 1Password SSH agent", { timeout: 240_000 }, async () => {
    const result = await runPayJson(
      configPath,
      secretRef,
      `${X402_NFT_BASE_URL}/mint/attractor?size=64&seed=${uniqueSeed()}`,
